...
#define prllarr(arr) for(ll i=0;i<arr.size();i++)cout<<arr[i]<<" ";
...
vll prefixSum(vll&arr){
    vll psum(arr.size(),0);
    psum[0]=arr[0];ll n=arr.size();

    for(ll i=1;i<n;i++)psum[i]=psum[i-1]+arr[i];
    return psum;
}
/*
⣿⣿⣿⣿⣿⣿⡷⣯⢿⣿⣷⣻⢯⣿⡽⣻⢿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣿⣇⠸⣿⣿⣆⠹⣿⣿⢾⣟⣯⣿⣿⣿⣿⣿⣿⣽⣻⣿⣿⣿⣿⣿⣿⣿
...
return 1;
}bool isVM(ll n, vector<ll>& arr, ll s, ll a) {
    vector<ll> na = arr;
    na[n - 1] += a;
    ll ns = s + a;
    double avg = ns / (2.0 * n);
